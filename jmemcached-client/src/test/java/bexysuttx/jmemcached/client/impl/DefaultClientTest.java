...
public class DefaultClientTest {
	...
	public void putSimple() throws IOException {
		...
		// verify(requestConverter).writeRequest(same(out), equalTo(new
		// Request(Command.PUT, key, null, array)));
	}
	...
	public void putFull() throws IOException {
		...
		verify(objectSerializer).toByteArray(value);
		// verify(requestConverter).writeRequest(same(out), equalTo(new
		// Request(Command.PUT, key, 1L, array)));
	}
	...
	/*
	 * private Request equalTo(final Request request) { return argThat(new
	 * ArgumentMatcher<Request>() {
	 * 
	 * @Override public boolean matches(Request argument) { return
